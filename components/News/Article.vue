<template>
    ...
    <nuxt-link rel="noopener noreferrer" :to="link" class="max-w-sm mx-auto group hover:no-underline w-full focus:no-underline dark:bg-zpr_purple-900">
        <nuxt-img provider="imagekit" format="webp" role="presentation" class="object-contain w-full rounded h-44 dark:bg-gray-500" :src="imgSrc" alt="" @error="imgFallback" />
        <div class="p-6 space-y-2">
            <h3 class="text-xl font-semibold 
            text-zpr_purple-second group-hover:text-zpr_purple-500 uppercase group-focus:text-zpr_purple-500">{{currentArticle.data.title}}</h3>
            <span class="text-xs dark:text-gray-400">{{articleDate}}</span>
            <p class="text-gray-100">{{currentArticle.data.txt}}</p>
        </div>
    </nuxt-link>
</template>
